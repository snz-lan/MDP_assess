...
def value_iteration(states, actions, transition_probs, rewards, gamma=0.9, max_iter=1000):
    ...
    """
    # Initializing value function arbitrarily (all zeros)
    V = {s: 0 for s in states}
    
    # Iterating a given fixed number of times
    for iteration in range(max_iter):
        V_new = {}
        for s in states:
            # Computing Q-value for each action for state s
            q_values = []
            for a in actions:
                # Calculating Q(s, a) = reward + gamma * expected future value
                q_value = rewards.get((s, a), 0) + gamma * sum(
                    transition_probs.get(s, {}).get(a, {}).get(s_prime, 0) * V[s_prime]
                    for s_prime in states
                )
                q_values.append(q_value)
            # this is the new value for state s is the maximum Q-value across all actions
            V_new[s] = max(q_values)
        V = V_new  # updating V after each iteration
    
    # extracting the policy using the final value function
    policy = {}
    for s in states:
        best_action = max(
            actions,
